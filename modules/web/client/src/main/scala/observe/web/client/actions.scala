...
object actions {
  ...
  // Actions
  final case class NavigateTo(page: ObservePages)       extends Action
  final case class NavigateSilentTo(page: ObservePages) extends Action
  ...
  final case class CloseUserPromptBox(result: UserPromptResult) extends Action
  ...
  final case class RequestGracefulStop(id: Observation.Id, observer: Observer, step: StepId)
      extends Action
  final case class RequestAbort(idNam: Observation.IdName, observer: Observer, step: StepId)                    extends Action
  final case class RequestObsPause(id: Observation.Id, observer: Observer, step: StepId)                 extends Action
  final case class RequestGracefulObsPause(id: Observation.Id, observer: Observer, step: StepId)         extends Action
  ...
  final case class RunResource(idN: Observation.IdName, step: StepId, resource: Resource) extends Action
  ...
  final case class ClearAllResourceOperationsOnStepChange(s: Observation.Id, step: StepId)
      extends Action
  ...
  final case class ServerMessage(e: ObserveEvent)       extends Action
  ...
  final case class UpdateSessionQueueTableState(s: TableState[SessionQueueTable.TableColumn])
      extends Action
  final case class UpdateStepTableState(id: Observation.Id, s: TableState[StepsTable.TableColumn])
      extends Action
  final case class UpdateSelectedStep(id: Observation.Id, step: StepId)      extends Action
    ...
    case s @ ServerMessage(u @ ObserveModelUpdate(view)) =>
      val someSteps   = view.sessionQueue.map(s =>
        (s"id: ${s.idName.name}",
         ...
         s.steps
           .filter(x =>
             List(StepState.Pending, StepState.Running, StepState.Aborted).exists(_ === x.status)
           )
