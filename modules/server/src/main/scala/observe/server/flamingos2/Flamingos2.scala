...
object Flamingos2 {
  ...
  def ccConfigFromSequenceConfig(config: CleanConfig): Either[ObserveFailure, CCConfig] =
    (for {
  ...
  def dcConfigFromSequenceConfig(config: CleanConfig): Either[ObserveFailure, DCConfig] =
    (for {
    ...
    } yield DCConfig(p, q, r, s)).leftMap(e => ObserveFailure.Unexpected(ConfigUtilOps.explain(e)))

  def fromSequenceConfig[F[_]](
    config: CleanConfig
  ): Either[ObserveFailure, Flamingos2Config] = for {
