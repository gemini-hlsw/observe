...
trait NiriEncoders {
  ...
  given EncodeEpicsValue[Mask, JMask] = EncodeEpicsValue {
    ...
    case Mask.MASK_7 | Mask.MASK_8 => JMask.Polarimetry
  ...
  def apply[F[_]: Async](
    ...
    new NiriController[F] {
      ...
      private def failOnDHSNotConected: F[Unit] =
        actOnDHSNotConected(
          ObserveFailure.Execution("NIRI is not connected to DHS").raiseError[F, Unit]
        )

      private def failOnArrayNotActive: F[Unit] =
        actOnArrayNotActive(
          ObserveFailure.Execution("NIRI detector array is not active").raiseError[F, Unit]
        )
      ...
      override def applyConfig(config: NiriController.NiriConfig): F[Unit] = {
        ...
        val cfgActions  =
          if (paramsDC.isEmpty) cfgActions1
          else
            cfgActions1 *>
      ...
      override def observe(fileId: ImageFileId, cfg: DCConfig): F[ObserveCommandResult] =
        L.debug(s"Start NIRI observe, file id $fileId") *>
          ...
          calcObserveTimeout(cfg).flatMap(epicsSys.observeCmd.post(_).flatTap { _ =>
