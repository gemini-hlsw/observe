...
class WebSocketHandler[M](modelRW: ModelRW[M, WebSocketConnection])
  ...
  def webSocket: Future[Action] = Future[Action] {
    ...
    val url      = s"$protocol://$host/api/observe/events"
    val ws       = new WebSocket(url)

    def onOpen(): Unit = {
      logger.info(s"Connected to $url")
      ObserveCircuit.dispatch(Connected(ws, 0))
    }
    ...
    def onMessage(e: MessageEvent): Unit =
      e.data match {
        case buffer: ArrayBuffer =>
          ...
          Either.catchNonFatal(Unpickle[ObserveEvent].fromBytes(byteBuffer)) match {
            case Right(event: ServerLogMessage)         =>
              ObserveCircuit.dispatch(AppendToLog(event))
            case Right(event: ObservationProgressEvent) =>
              ObserveCircuit.dispatch(ServerMessage(event))
            case Right(event)                           =>
  ...
  def connectedHandler: PartialFunction[Any, ActionResult[M]] = { case Connected(ws, delay) =>
    ...
    val handle = setInterval(5.minutes)(ObserveCircuit.dispatch(VerifyLoggedStatus))
      ...
      Effect.action(
        AppendToLog(
          ServerLogMessage(ServerLogLevel.ERROR,
                           Instant.now,
                           "Error connecting to the observe server"
          )
        )
      )
    )
