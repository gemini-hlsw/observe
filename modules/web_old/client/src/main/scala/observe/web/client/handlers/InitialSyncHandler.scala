...
class InitialSyncHandler[M](modelRW: ModelRW[M, InitialSyncFocus])
  ...
  def runningSequence(s: ObserveModelUpdate): Option[SequenceView] =
    s.view.sessionQueue.filter(_.status.isRunning).sortBy(_.obsId.name).headOption
  ...
  def defaultPage(
    ...
    val effect = loaded.headOption.flatMap { id =>
      s.sessionQueue.find(_.obsId === id).map { s =>
  ...
  def handle: PartialFunction[Any, ActionResult[M]] = {
    ...
    case ServerMessage(s: ObserveModelUpdate) if value.firstLoad =>
      ...
      val sids             = s.view.sessionQueue.map(_.obsId)
      ...
      updatedLE(
        Focus[InitialSyncFocus](_.firstLoad).replace(false) >>> Focus[InitialSyncFocus](
          _.displayNames
        ).replace(
          storedDisplayNames
        ) >>> update,
